...
Begin Window winSmsCommandsList
	...
	Begin PushButton btnReload
		...
		Width				=	90
	End
	Begin Listbox smsCommandsListBox
		AllowAutoDeactivate=	True
		...
		Index				=	-2147483648
		InitialParent	=	""
		InitialValue	 =	"Account Sid	Sim Sid	Sid	Payload	Status	Created"
		Italic			 =	False
		Left				=	38
		LockBottom		=	True
		LockedInPosition=	False
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub Load(simSids() as String)
			...
			for each simSid as String in simSids
				selectedSimSIDs.add(simSid)
				readRequestSimSIDs.add(simSid)
			next simSid
			
			//selectedSimSIDs = simSids // by reference https://blog.xojo.com/2019/09/18/working-with-arrays-assignment-vs-copy/
			//readRequestSimSIDs = selectedSimSIDs // by reference https://blog.xojo.com/2019/09/18/working-with-arrays-assignment-vs-copy/
	...
	#tag Method, Flags = &h21
		Private Sub FetchSmsCommands()
			...
			// fetch the smscommands for each sim sid 
			//	the spinner spins until all sms commands are fetched amd the list loaded
			//	how:
			...
			//		the oClient request success event will call the function(Twilio/Client.SimFetchSMSCommandsResponse) that 
			...
			//				if there are, it will call oClient.FetchSMSCommands("", simSid) once more, with the new first item on the readRequestSimSIDs as simSid
			//				if there are no more items to process, stop the spinner and re-enable the reload button
			//		if there is an error and it can be retried, the oClient request error event will call the function that
			//			will add the sim sid back to the readRequestSimSIDs and retry the request?
			//  load the sms commands to the list(LoadSmsCommandsList)
	...
	#tag Method, Flags = &h21
		Private Sub LoadSmsCommandsList()
			System.DebugLog("winSmsCommandsList LoadSmsCommandsList called.")
			 // Clear
			 smsCommandsListBox.RemoveAllRows
			 
			 // Correct sort indicator
			 smsCommandsListBox.HasHeader = false
			 smsCommandsListBox.SortingColumn = 5
			 smsCommandsListBox.ColumnSortDirectionAt(5) = Listbox.SortDirections.Descending
			 smsCommandsListBox.HasHeader = true
			 var tiScrollPosition as Integer = smsCommandsListBox.ScrollPosition
			 
			 // Load Listbox
			 for each simCommand as Twilio.SmsCommand in simsSmsCommandsList
				
				// Add row
			 	var smsCommandRowData() as String
				
				smsCommandRowData.Add(simCommand.sAccountSID)
				smsCommandRowData.Add(simCommand.simSID)
				smsCommandRowData.Add(simCommand.sID)
				smsCommandRowData.Add(simCommand.payload)
				smsCommandRowData.Add(simCommand.status)
				smsCommandRowData.Add(simCommand.dtmCreated.ToString)
				
				// Add row and retain objects
				smsCommandsListBox.AddRow(smsCommandRowData)
				smsCommandsListBox.RowTagAt(smsCommandsListBox.LastAddedRowIndex) = simCommand
								
			 next simCommand
			 
			 // Restore position
			 smsCommandsListBox.ScrollPosition = tiScrollPosition
			 smsCommandsListBox.Enabled = true
...
#tag Events btnReload
	#tag Event
		Sub Action()
			...
			for each simSid as String in selectedSimSIDs
				System.DebugLog("winSmsCommands btnReload simSid: " + simSid)
				readRequestSimSIDs.add(simSid)
			next simSid
			FetchSmsCommands
		End Sub
...
#tag Events oClient
	#tag Event
		Sub SimFetchSMSCommandsComplete(simSID as String)
			...
			System.DebugLog("winSmsCommandsList SimFetchSMSCommandsComplete called for sim '" + simSID + "'." )
			...
			For Each smsCommand As Twilio.SmsCommand In me.smsCommands
				System.DebugLog("winSmsCommandsList SimFetchSMSCommandsComplete Sms Command sid: " + smsCommand.sID)
			...
			Next smsCommand
		 	...
		 	if( readRequestSimSIDs.Count = 0 ) then
				...
				System.DebugLog("winSmsCommandsList SimFetchSMSCommandsComplete ALL sims processed!" )
				btnReload.Enabled = true
				pwWait.Visible = false 
				// load items to List
				LoadSmsCommandsList
			else
				System.DebugLog("winSmsCommandsList " + readRequestSimSIDs.Count.ToString + " more sims to process.")
				FetchSmsCommands	
			end
